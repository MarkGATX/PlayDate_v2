export async function getActivitiesFromWeather(
  latitude: number,
  longitude: number,
  weatherCode: number | undefined,
) {
  const goodWeatherCodes: number[] = [800, 801, 802, 803, 804, 741];
  const goodWeatherSearches: string[] = [
    "playground%20",
    "hike%20",
    "lake%20",
    "zoo%20",
    "ice%20cream%20",
    "ice%20cream%20parlor%20",
    "state%20park%20",
    "play",
  ];
  const badWeatherSearches: string[] = [
    "museum%20",
    "movie%20",
    "library%20",
    "craft%20",
  ...
  const minLatitude: number = latitude - 0.5;
  const maxLatitude: number = latitude + 0.5;
  const minLongitude: number = longitude - 0.5;
  const maxLongitude: number = longitude + 0.5;
  ...
  if (weatherCode) {
    if (goodWeatherCodes.includes(weatherCode)) {
      ...
      for (let i = goodWeatherIndexArray.length - 1; i > 0; i--) {
        ...
        [goodWeatherIndexArray[i], goodWeatherIndexArray[j]] = [
          goodWeatherIndexArray[j],
          goodWeatherIndexArray[i],
        ];
      }
      ...
      for (let i = badWeatherIndexArray.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [badWeatherIndexArray[i], badWeatherIndexArray[j]] = [
          badWeatherIndexArray[j],
          badWeatherIndexArray[i],
        ];
      }
      for (let i = 0; i < 5; i++) {
