...
export default function UpdatePlaydateStatus(
  notification: NotificationDetailsType,
) {
  const { sender, kid, notification_type, receiver, id, playdate_id } =
    notification;
  const [playdateStatus, setPlaydateStatus] =
    useState<PlaydateAttendanceType | null>(null);

  useEffect(() => {
    const getUpdatedPlaydateStatus = async () => {
      try {
        const {
          data: playdateStatusData,
          error: playdateStatusDataError,
        }: {
          data: PlaydateAttendanceType[] | null;
